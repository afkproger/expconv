...
class EffectivenessCalculator:
    ...
    def __calculate_paired_indicators(self, data, n):
        ...
        #
        # # Для каждой пары индексов считаем произведение соответствующих столбцов
        # result = np.column_stack([data[:, i] * data[:, j] for i, j in pairs])
        # return result
    ...
    def __get_fn_combinations(self):
        ...
        # Формируем список строк 'j1', 'j2', ..., 'jn'
        fn_labels = [f"j{i + 1}" for i in range(count_factors)]

        # Создаем комбинации от одиночных 'j1', 'j2', ..., до 'j1*j2', 'j1*j2*j3', и т.д.
        all_combinations = []
        for r in range(1, count_factors + 1):
            combinations = itertools.combinations(fn_labels, r)
            for combo in combinations:
                # Добавляем знак умножения перед одиночными и комбинированными
                all_combinations.append('*' + '*'.join(combo))

        return all_combinations

    def get_fn_labels(self):
        fn_labels = self.__get_fn_combinations()
        return fn_labels
        # return self.__generate_formula(fn_labels)

    @staticmethod
    def form_polynomial(coefficients, labels):
        # Первый элемент - это свободный член
        polynomial = f"{coefficients[0]}"

        # Проходим по остальным коэффициентам и соответствующим меткам
        for coef, label in zip(coefficients[1:], labels):
            # Добавляем в строку: 'коэффициент * метка'
            if coef >= 0:
                polynomial += f" + {coef}{label}"
            else:
                polynomial += f" - {-coef}{label}"
